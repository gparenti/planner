...
def add():
    head = request.form['todo_head'].strip()  # Input for title
    body = request.form['todo_body'].strip()  # Input for body

    if not head or not body:  # Check if either input is empty
        flash('Planbeschreibung und Titel dürfen nicht leer sein!')
        return redirect(url_for('index'))

    todo = Todo(head=head, body=body, complete=False, user_id=current_user.id)
    db.session.add(todo)
    db.session.commit()
...
def register():
   ...
   if form.validate_on_submit():
      ...
      db.session.add(user)
      db.session.commit()
      flash('Glückwunsch du hast dich erfolgreich registriert!')
      return redirect(url_for('login'))
...
#Eigene Entwicklung - Aufgabe abschliessen
@app.route('/complete/<id>', methods=['GET','POST'])
@login_required
def complete(id):
    todo = Todo.query.filter_by(id=int(id)).first()

    # Prüfen, ob der Benutzer, der die Aufgabe zu erledigen versucht, der Eigentümer der Aufgabe ist
    if todo.user_id == current_user.id:
        ...
        flash("Du darfst diesen Plan nicht schliessen!")
        return redirect(url_for('explore'))

#Eigene Entwicklung - Plan wiedereröffnen
...
def notcomplete(id):
    ...
    if todo.user_id == current_user.id:
        ...
        flash("Du darfst diesen Plan nicht wiedereröffnen!")
        return redirect(url_for('explore'))

#Eigenen Entwicklung - Plan löschen
...
def deletetask(id):
    ...
    if todo.user_id == current_user.id:
        ...
        flash("Du darfst diesen Plan nicht löschen!")
        return redirect(url_for('explore'))
...
def edittask(id):
    ...
    if todo.user_id == current_user.id:
        ...
        flash("Du darfst diesen Plan nicht bearbeiten!")
        return redirect(url_for('explore'))
